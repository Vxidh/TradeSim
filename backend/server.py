...
symbol_to_asset_id_map = {
    'RELIANCE': 1, 'AAPL': 2, 'MSFT': 3, 'GOOG': 4,
    'AMZN': 5, 'META': 6, 'TSLA': 7
}
asset_id_to_symbol_map = {v: k for k, v in symbol_to_asset_id_map.items()}
...
def get_trades():
    ...
    result = [
        {
            'trade_id': t.trade_id,
            'symbol': asset_id_to_symbol_map.get(t.asset_id, 'UNKNOWN'),
            'price': t.price,
            'quantity': t.quantity,
            'side': 'N/A',
...
def place_order():
    ...
    if not target_asset_id:
        return jsonify({'error': 'Unknown symbol for asset mapping'}), 400
